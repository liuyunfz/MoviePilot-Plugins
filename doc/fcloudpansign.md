...
**1.1.2 固定官方站点与公共应用，使用 RFC 8628 Device Authorization Grant。需要 F-Cloudpan 服务端支持 `PUBLIC_DEVICE` 公共应用。** 旧版只支持机密应用的云盘不能直接使用本版本。
...
2. 未授权时只显示连接和网络设置。默认使用 MoviePilot 已配置的代理（未配置时直连）；如需修改代理或超时，先点击“保存”。
3. 点击设置页底部“查看数据”→“连接云盘账号”，本页会生成“前往 F-Cloudpan 授权”按钮。点击按钮打开云盘。设置页也保留“保存后连接 / 重新授权”兼容入口。也可在另一台设备打开云盘确认页面，输入显示的确认码。
4. 在云盘登录，核对应用名称、确认码与权限，点击允许。插件在后台完成连接；重新打开插件详情查看结果，无需浏览器回调 MP。
5. 授权成功后重新打开设置页，即可看到签到方式、周期、通知等配置。启用定时签到，或选择“保存后立即签到一次”。授权完成本身只读取资料，不立即签到；之前已启用的定时计划会恢复。
...
- **取消本次连接**：在“查看数据”中点击按钮立即执行，无需保存。停止本地等待并移除设备码，已有账号授权保持不变。该操作不撤销已发出的云端确认码；不要再确认它，等待其过期即可。
- **断开本设备授权**：在“查看数据”中展开“断开本设备授权…”并点击确认按钮立即执行，无需保存。撤销当前设备授权，成功后清除本地资料和历史，不影响其他 MP 设备。个人中心的“撤销整个应用”会断开该用户在该应用下的全部设备。撤销未确认则停止账户操作并保留凭据，允许再次撤销或在云盘个人中心撤销。
...
插件新增本机 `POST /api/v1/plugin/FCloudpanSign/action`，供数据页按钮使用，要求 MoviePilot 登录且为激活的管理员；请求动作仅支持 connect、cancel、revoke。断开还需 confirmed=true。没有匿名入口，也不新增云盘回调地址。按钮执行时显示处理中，完成后刷新当前页；失败会在连接状态中显示。
...
- 连接未开始且没有授权链接：申请设备码的请求未成功。1.1.2 默认使用 MP 代理；检查 NAS 容器是否能连到代理及 `https://fcloudpan.com`，容器内的 `127.0.0.1` 通常不指向 NAS 宿主机。新版会区分代理、DNS、TLS、连接超时和响应超时，并标出使用代理还是直连。修改网络配置后重新打开连接开关并保存，不会自动重试申请。
...
1.1.2 修复已用 MoviePilot 原版 `FormRender.vue`、`PageRender.vue` 和 Vuetify 配置进行本地验证：内置图标、代理开关，以及本地测试账号的连接、取消、确认断开和页面刷新；定向测试覆盖管理员鉴权、未授权禁用及授权后恢复定时计划。该验证不代表已在用户 NAS 上完成生产授权。
