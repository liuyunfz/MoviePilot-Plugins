...
**1.2.0 固定官方站点与公共应用，使用 RFC 8628 Device Authorization Grant。需要 F-Cloudpan 服务端支持 `PUBLIC_DEVICE` 公共应用。** 旧版只支持机密应用的云盘不能直接使用本版本。
...
1.2.0 使用 MoviePilot 的 Vue 联邦组件（Vue 3.5 / Vuetify 3.7 及以上兼容版本），设置页负责授权和配置，数据页只展示账号资料及签到记录。请完整安装包含 `dist/assets` 的插件目录；用户无需安装 Node.js 或编译前端。

## 安装与连接

1. 在 MoviePilot 添加本仓库并安装 **F-Cloudpan 签到**。
2. 打开插件即可看到连接和网络设置；如需调整代理或超时，先点击“保存设置”。默认使用 MoviePilot 配置的代理，未配置时直连。
3. 在设置页点击“连接云盘账号”，再点击“打开授权窗口”。采用与 MP 的 115 存储授权相同的居中 600×700 浏览器弹窗；浏览器拦截弹窗时可使用新标签页链接。无需另行保存授权开关。
4. 在云盘登录，核对应用名称、确认码与权限，点击允许。插件界面会自动显示结果；切回 MoviePilot 也会立即检查，无需刷新或重新打开插件页。
5. 授权成功后弹窗自动关闭，设置页自动展示签到设置；点击“查看数据”可查看账号资料和记录。调整周期、签到方式和通知后点击“保存设置”。授权本身不立即签到；之前已启用的定时计划会恢复。

MP 的 NAS / 容器只需能主动连接 `https://fcloudpan.com`；浏览器打开同一站点完成登录与确认。用户不必申请自己的应用，也无需配置 MP 公网地址。
...
- **取消本次连接**：在设置页的连接管理中点击按钮立即执行，无需保存。停止本地等待并移除设备码，已有账号授权保持不变。该操作不撤销已发出的云端确认码；不要再确认它，等待其过期即可。
- **断开本设备授权**：在设置页的连接管理中展开“断开本设备授权…”并点击确认按钮立即执行，无需保存。撤销当前设备授权，成功后清除本地资料和历史，不影响其他 MP 设备。个人中心的“撤销整个应用”会断开该用户在该应用下的全部设备。撤销未确认则停止账户操作并保留凭据，允许再次撤销或在云盘个人中心撤销。
- **重新授权**：成功后替换账号并清除旧资料和历史。旧云端授权不会因为本地替换自动消失，可在云盘个人中心撤销。
- 维护者变更内置应用时会清除旧本地身份，需要重新连接；应先在云盘撤销旧授权。

未授权、应用到期、撤销未确认或刷新结果未知时，隐藏签到设置和账号数据，停止定时签到调用。网络设置仍可修改；已有周期、签到方式等配置保留，重新授权后恢复。

令牌仅保存在 MoviePilot 的插件存储中，不出现在表单、详情、日志、历史、通知或源码。设备码仅供 MP 后台使用，页面只展示用户确认码。不要复制同一授权存储给多个 MP 进程同时运行；各实例应分别授权。

## 从旧版升级

从 1.1.2 升级保留已有账号授权和设置。首次升级后可刷新一次 MoviePilot，以加载新的界面组件；之后每次授权都无需手工刷新。旧 PNG 可能已在目录安装时损坏，1.2.0 不再读取它：同一张 Telegram 原图以文本数据分别内置在 Python 资源和前端构建产物中，图片字节与原图一致。
...
插件新增本机 `POST /api/v1/plugin/FCloudpanSign/action`，供数据页按钮使用，要求 MoviePilot 登录且为激活的管理员；请求动作仅支持 connect、cancel、revoke。断开还需 confirmed=true。授权和状态接口均不允许匿名访问，也不新增云盘回调地址。按钮执行时显示处理中，完成后刷新当前页；失败会在连接状态中显示。

`GET /api/v1/plugin/FCloudpanSign/view` 同样要求 MP 激活管理员登录，仅读取缓存的界面状态和非敏感配置，响应禁止缓存。等待授权或一次性任务期间每 3 秒检查本地状态，成功后停止定时检查；页面隐藏时暂停、切回时立即检查、关闭时清理计时器和请求。网络失败退避至最多 30 秒。界面更新不会覆盖尚未保存的输入，也不会额外访问云盘令牌接口。前端构建产物通过 MP 自带插件静态文件接口加载，不包含用户凭据。
...
公共客户端请求不带 Basic 或 Secret。接口不跟随重定向，不关闭 TLS 校验，通过显式参数使用 MoviePilot 代理（可在插件中关闭），不读取环境变量代理、`.netrc` 或用户 Cookie。确认链接必须与内置官方站点同源。

- 连接未开始且没有授权链接：申请设备码的请求未成功。1.1.2 默认使用 MP 代理；检查 NAS 容器是否能连到代理及 `https://fcloudpan.com`，容器内的 `127.0.0.1` 通常不指向 NAS 宿主机。新版会区分代理、DNS、TLS、连接超时和响应超时，并标出使用代理还是直连。修改网络配置并保存后，再点击连接按钮；不会自动重试申请设备码。
- 设置页图标：使用插件内置的 Telegram 原图，通过原生图片标签显示，不依赖浏览器访问 GitHub；市场列表图标仍取自仓库地址。
...
历史 1.1.2 曾用 MoviePilot 原版 `FormRender.vue`、`PageRender.vue` 和 Vuetify 配置进行本地验证：内置图标、代理开关，以及本地测试账号的连接、取消、确认断开和页面刷新；定向测试覆盖管理员鉴权、未授权禁用及授权后恢复定时计划。该验证不代表已在用户 NAS 上完成生产授权。


1.2.0 进一步模拟 MP 目录安装的 UTF-8 文本写入，再加载真实 Vue 联邦构建产物验证图标、等待授权到成功的自动变化、未保存配置保留及设置保存。仍需在用户 NAS 升级后确认，不能以本地预览代替真实安装验收。

维护者构建前端（在 `web/fcloudpansign` 目录）：
...
提交源码、锁文件与生成的 `plugins/fcloudpansign/dist/assets`，然后更新安装 ZIP。不要将 `node_modules` 放进插件目录或安装包。

头像使用云盘 API 返回的实际地址；以 `/` 开头的地址按官方站点根地址补全。空值或加载失败时显示昵称首字作为默认头像，避免空白破图。云盘默认头像原先未通过 API 返回，配套服务端已补齐按原头像种子计算的路径，需服务端发布后才能与云盘默认头像一致。
